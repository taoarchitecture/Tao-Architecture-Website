...
function resolveRange(query: { since?: string; until?: string }) {
  const today = new Date();
  const defaultSince = new Date(today);
  defaultSince.setDate(today.getDate() - 6);

  return {
    since: toDateString(query.since, defaultSince),
    until: toDateString(query.until, today),
  };
}
...
export function getObservabilityStatusPayload() {
  const config = getConfigStatus();
  ...
  if (!config.vercelTokenConfigured) {
    ...
    nextSteps.push('Create a Vercel access token with project access and store it as `VERCEL_ACCESS_TOKEN` in `.env.local`.');
  }
  ...
  if (!config.vercelProjectIdConfigured) {
    ...
    nextSteps.push('Add the Vercel project identifier as `VERCEL_PROJECT_ID` in `.env.local`.');
  }
...
export async function getWebAnalyticsOverviewPayload(query: { since?: string; until?: string }) {
  const config = getConfigStatus();

  if (!config.webAnalyticsQueryReady) {
    const error: any = new Error('Vercel Web Analytics is not fully configured on the server.');
    error.missing = [
